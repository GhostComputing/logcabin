...
(no servers are involved):
    ...
    build/logcabin/bin/SmokeTest --mock && echo 'Smoke test completed successfully'
...
itself:
    ...
    build/logcabin/bin/LogCabin --config logcabin-1.conf --bootstrap
...
start this server (leave it running):
    ...
    build/logcabin/bin/LogCabin --config logcabin-1.conf
...
Let's start up the second server in another terminal (leave it running):
    ...
    build/logcabin/bin/LogCabin --config logcabin-2.conf
...
keep you from doing anything stupid):
    ...
    build/logcabin/bin/LogCabin --config logcabin-3.conf
...
cluster:
    ...
    build/logcabin/bin/Reconfigure --cluster=$ALLSERVERS set 127.0.0.1:5254 127.0.0.1:5255 127.0.0.1:5256
...
Finally, you can run a LogCabin client to exercise the cluster:
    ...
    build/logcabin/bin/HelloWorld --cluster=$ALLSERVERS
...
TreeOps exposes LogCabin's data structure on the command line:
    ...
    build/logcabin/bin/TreeOps --cluster=$ALLSERVERS dump
